...
```{r} 
source("~/sparklyr_exercise/ezequiel_paura/salesData.R")
```
...
1. Data preparation
```{r}
sales_sdf<-sales_sdf <- spark_read_csv(sc, "sales", "../future_sales_data/sales_train.csv.gz")
sales_sdf %>%
  mutate(dt=to_timestamp(unix_timestamp(date, 'dd.MM.yyyy'))) %>%
  mutate(year=year(dt), month=month(dt)) %>%
  select(-dt) ->
...
sales_sdf %>% 
  ...
  summarise(total_sales=sum(item_cnt_day, na.rm=T))-> monthly_sdf

sdf_register(monthly_sdf, "sales_monthly")
```
```{r}
library(DBI)
dbGetQuery(sc, "SELECT * FROM sales_monthly LIMIT 10")
```
...
```{r}
train_sdf %>% 
  ml_linear_regression((total_sales ~ shop_id + year +month ))-> model 
```
